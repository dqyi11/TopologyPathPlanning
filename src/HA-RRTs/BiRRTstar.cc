...
RRTNode* BiRRTstar::init( POS2D start, POS2D goal, COST_FUNC_PTR p_func, double** pp_cost_distribution ) {
    if( _p_st_root ) {
        delete _p_st_root;
        _p_st_root = NULL;
    }
...
KDNode2D BiRRTstar::_find_nearest( POS2D pos ) {
    KDNode2D node( pos );
...
std::list<KDNode2D> BiRRTstar::_find_near(POS2D pos) {
...
RRTNode* BiRRTstar::_create_new_node(POS2D pos) {
...
void BiRRTstar::_attach_new_node(RRTNode* p_node_new, RRTNode* p_nearest_node, std::list<RRTNode*> near_nodes) {
...
void BiRRTstar::_rewire_near_nodes(RRTNode* p_node_new, std::list<RRTNode*> near_nodes) {
    for( std::list<RRTNode*>::iterator it=near_nodes.begin(); it!=near_nodes.end(); it++ ) {
        RRTNode * p_near_node = (*it);
...
bool BiRRTstar::_get_closet_to_goal( RRTNode*& p_node_closet_to_goal, double& delta_cost ) {
    bool found = false;
